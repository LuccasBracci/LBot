...
#     # for member in wern.get_all_members():
#     #     if member.activity and member.activity.name == 'League of Legends':
#     await ctx.print(member.activity)
...
async def tq(ctx):
    ...
    pVal()
    if p:
        await ctx.send("Quotes toggled on")
    else:
        await ctx.send("Quotes toggled off")


@wern.command()
async def cope(ctx):
    await ctx.send("Cope with this retard. You want to see peak male performance? Weap for me.",
